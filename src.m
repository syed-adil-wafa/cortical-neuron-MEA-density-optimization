...
% Segregate features by electrophysiological group
...
% Concatenate electrophysiological groups
plots.all = catpad(1, plots.firing, plots.bursting, plots.nbursting, plots.synchrony);

% Define titles for each electrophysiological group
plots.titles = {'Firing at different cell densities';...
    'Bursting at different cell densities';...
    'Network bursting at different cell densities';...
    'Synchrony at different cell densities'};

% Define figure dimensions for each electrophysiological group
plots.dims = [2,3; 4,5; 4,4; 2,3];

for fig = 1:size(plots.all, 1)
    
    % Isolate electrophysiological group
    plots.plot_feature = plots.all(fig, ~isnan(plots.all(fig, :)))';
    
    figure; hold on; box off % figure settings
    suptitle(plots.titles(fig, :)); % figure title
    
    for feature = 1:size(plots.plot_feature, 1)
        
        % Configure figure settings
        subplot(plots.dims(fig, 1), plots.dims(fig, 2), feature);
        hold on; box on
